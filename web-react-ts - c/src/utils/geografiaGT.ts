/**
 * Catálogo oficial de departamentos y municipios de Guatemala (22 departamentos).
 * Fuente: División política-administrativa de la República de Guatemala.
 *
 * Usado para selects enlazados (al cambiar el departamento, se repuebla el
 * select de municipios). Esto reemplaza los campos abiertos, evita erratas
 * y simplifica filtros/agrupaciones en vistas del comité.
 */
export const DEPARTAMENTOS_GT: Record<string, string[]> = {
  'Alta Verapaz': [
  ...
  'Jutiapa': [
    ...
    'Comapa', 'Jalpatagua', 'Conguaco', 'Moyuta', 'Pasaco',
  ...
  'San Marcos': [
    ...
    'El Tumbador', 'El Rodeo', 'Malacatán', 'Catarina', 'Ayutla',
    'Ocós', 'San Pablo', 'El Quetzal', 'La Reforma', 'Pajapita',
...
export const LISTA_DEPARTAMENTOS = Object.keys(DEPARTAMENTOS_GT).sort();

export function municipiosDe(departamento: string): string[] {
  return DEPARTAMENTOS_GT[departamento] ?? [];
}

export function esMunicipioValido(departamento: string, municipio: string): boolean {
